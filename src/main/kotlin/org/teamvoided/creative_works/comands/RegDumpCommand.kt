...
object RegDumpCommand {
    fun init(dispatcher: CommandDispatcher<ServerCommandSource>) {
        ...
        val registry = registryArg().executes { regdump<Any>(it, getRegistry(it)) }.buildChildOf(root)
        regEntryArg().executes { regdump<Any>(it, getRegistry(it), getEntry(it)) }.buildChildOf(registry)
    ...
    fun <T> regdump(
        ...
        val codec = (REG_LIST[registry.key]) as Codec<T>?
        ...
        if (entryId != null) {
            ...
            src.message(" Entry: $entryId ")
            codec?.encodeStart(ops, entry as T)?.ifError { src.error(" Error while encoding $it") }?.ifSuccess {
                src.sendSystemMessage(Text.literal(" ${gson.toJson(it)}").styled { s -> s.withColor(SECONDARY_COLOR) })
            }
            return 1
        }
        if (codec == null) {
        ...
        val list = registry.entries.associate { it.key.value to codec.encodeStart(ops, it.value as T) }
        src.dumpResources(world, id, list)

        return 1
    }

    fun painAndSuffering(ctx: CommandContext<ServerCommandSource>, extra: Identifier, entryId: Identifier? = null): Int {
        val src = ctx.source ?: return 0
        val world = src.world ?: return 0
        val dynReg = world.registryManager ?: return 0
    ...
    fun ServerCommandSource.dumpResources(
        world: ServerWorld, name: Identifier, list: Map<Identifier, DataResult<JsonElement>>
    ) {
        val folder = world.dumpFolder(name.fileFormat())
    ...
    fun ServerWorld.dumpFolder(name: String): File {
        ...
        if (regFolder.exists() && regFolder.isDirectory) regFolder.deleteRecursively()
        regFolder.mkdirs()
